...
export default function RegisterTherapistScreen({ navigation, route }) {
  const { register, setRole, loading } = useAuth();
  const selectedRole = useMemo(() => route?.params?.selectedRole || 'Therapist', [route?.params]);
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      navigation.navigate('OTPVerification', { role: 'Therapist' });
    } catch (err) {
      // handled in context
    }
  };
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 32 }}>
      <Text style={styles.title}>Registrierung Therapeut:in</Text>
      <Text style={styles.subtitle}>Bestätige deine Angaben und lade dein Zertifikat hoch.</Text>
      ...
      <Text style={styles.label}>Zertifikat hochladen</Text>
      <Pressable
        style={styles.upload}
        onPress={() => updateField('certificate', 'Therapie-Zertifikat.pdf')}
      >
        <Text style={styles.uploadText}>
          {form.certificate || 'Zertifikat hochladen (Platzhalter)'}
        </Text>
      </Pressable>
